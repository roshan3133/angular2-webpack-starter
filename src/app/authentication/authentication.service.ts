...
import { jsonHeader } from '../utils/headers/xhr-headers';
//import { Configuration } from '../utils/backendapi/backendapi';
//import Configuration = require("../utils/backendapi/backendapi");
//var config = new Configuration.LoginApiUrl;
//console.log(config)
...
export class AuthenticationService {
  ...
  public login(body: string) {
    ...
    console.log('loginAccount with:' + body + this.extractToken );
    return this.http.post("http://10.140.65.74:5050/login", body, jsonHeader())
    //return this.http.post("http://10.140.65.74:8080/api/loginuser", body, jsonHeader())
  ...
  public logout() {
    if (this.isAuthenticated()) {
      ...
      this.postResource('http://10.140.65.74:5050/', 'logout')
  ...
  private extractToken(res: Response) {
    ...
    if (res.status === 200) {
      ...
      //let maxTokenExpiryTime = Math.floor(new Date().getTime() / 1000) + Number(body[response][user]['token_age']);
      let maxTokenExpiryTime = Math.floor(new Date().getTime() / 1000) + Number(body['token_age']);
